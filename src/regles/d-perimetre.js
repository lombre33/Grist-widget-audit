...
export function analyserAccesAppat(journal, ctx) {
  ...
  return [constat({
    ...
    titre: "Le widget lit ou modifie une table sans qu'aucune déclaration de lecture étendue n'ait été détectée",
    constat: `${appels.length} appel(s) \`${methodes}\` observé(s) sur une table présente dans le document de test uniquement comme témoin, jamais sélectionnée, jamais annoncée par une colonne attendue. L'outil n'a détecté, dans le README, aucune formulation reconnue annonçant une lecture ou écriture portant sur l'ensemble du document (ex. « l'ensemble du document », « toutes les tables », ou leurs équivalents anglais) associée à un accès \`full\`. Ce rapprochement est un repérage textuel, pas une lecture exhaustive : une formulation différente y échapperait.`,
    impact: "Cette table n'est révélée que par une énumération complète du document (`listTables()` puis récupération systématique de chaque table trouvée) : c'est le comportement d'un widget qui aspire tout ce qu'il peut atteindre plutôt que de se limiter à ce que sa fonction déclarée justifie. Avec un accès `full` sur un vrai document Grist, la même logique atteint des tables réelles sans aucun rapport avec le widget — RH, paie, tout ce que l'agent n'a jamais imaginé lui exposer en installant ce widget précis. Si ce comportement est en réalité intentionnel et documenté dans des termes que ce repérage ne reconnaît pas, c'est ce texte qu'il faut rapprocher de la remédiation ci-dessous plutôt que d'écarter le constat.",
    remediation: "Si ce comportement est intentionnel, l'annoncer dans le README avec une formulation explicite — dans la même rubrique « Accès au document » où B-DOC-03 vérifie déjà le niveau d'accès demandé — par exemple « ce widget lit/écrit sur l'ensemble du document » (ou son équivalent anglais, « reads/writes the entire document », « all tables »). Sinon, restreindre le widget aux tables et colonnes qu'il déclare réellement utiliser (voir `columns` dans `grist.ready()`), et ne jamais énumérer aveuglément le document avec `listTables()` suivi d'une récupération systématique de chaque table trouvée.",
    preuve,
